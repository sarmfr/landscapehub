...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-8">
        <form action="{{ route('vendor.services.update', $service) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="6"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-600">{{ old('description', $service->description) }}</textarea>
            ...
            <div class="mb-6">
                ...
                <div class="flex gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="pricing_type" value="fixed" {{ old('pricing_type', $service->pricing_type) === 'fixed' ? 'checked' : '' }} onchange="togglePriceField()" class="w-4 h-4 text-green-600">
                        <span class="font-medium">Fixed Price</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="pricing_type" value="quote" {{ old('pricing_type', $service->pricing_type) === 'quote' ? 'checked' : '' }} onchange="togglePriceField()" class="w-4 h-4 text-green-600">
                        <span class="font-medium">Quote-Based</span>
                    </label>
                </div>
            </div>
            <div id="price-field" class="mb-6 {{ old('pricing_type', $service->pricing_type) === 'quote' ? 'hidden' : '' }}">
            ...
            <div class="flex gap-4">
                ...
                <form action="{{ route('vendor.services.destroy', $service) }}" method="POST" class="ml-auto" onsubmit="return confirm('Delete this service?');">
                    @csrf
                    @method('DELETE')
