...
<body class="bg-gray-100 font-sans">
    <header class="bg-white shadow-md sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <a href="{{ route('home') }}" class="text-2xl font-bold text-green-800 flex-shrink-0">LandScapeHub</a>
            ...
            <nav class="hidden md:flex items-center space-x-4 text-sm font-medium">
                ...
                <a href="{{ auth()->check() ? route('quotes.index') : route('login') }}" class="text-gray-700 hover:text-green-700 px-2 py-1 rounded transition {{ request()->routeIs('quotes*') ? 'text-green-700 font-semibold' : '' }}">Requests</a>
                ...
                <a href="{{ route('cart.view') }}" class="relative ml-2 font-semibold text-gray-700 hover:text-green-700 transition">
                    Cart
        ...
        <div id="mobile-menu" class="hidden md:hidden bg-white border-t px-6 py-4 space-y-3 text-sm">
            ...
            <a href="{{ auth()->check() ? route('quotes.index') : route('login') }}" class="block text-gray-700 hover:text-green-700">Quote Requests</a>
    ...
    <footer class="relative bg-green-950 text-white py-16 mt-16 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-12 relative z-10">
            <div>
                <h3 class="font-black text-2xl mb-6">LandScapeHub</h3>
            ...
            <div>
                ...
                <ul class="space-y-3 text-sm">
                    <li><a href="{{ route('products') }}" class="text-green-100 hover:text-white transition">Shop Products</a></li>
                    <li><a href="{{ route('services') }}" class="text-green-100 hover:text-white transition">Services</a></li>
                    <li><a href="{{ route('register') }}" class="text-green-100 hover:text-white transition">Become Vendor</a></li>
                </ul>
            </div>
            <div>
                <h4 class="font-black uppercase tracking-widest text-xs text-green-400 mb-6">Support</h4>
                <ul class="space-y-3 text-sm">
                    <li><a href="mailto:admin@landscapehub.com?subject=Help%20Center" class="text-green-100 hover:text-white transition">Help Center</a></li>
                    <li><a href="mailto:admin@landscapehub.com" class="text-green-100 hover:text-white transition">Contact Us</a></li>
                    <li><a href="{{ auth()->check() ? route('quotes.index') : route('login') }}" class="text-green-100 hover:text-white transition">Request a Quote</a></li>
                </ul>
            </div>
            <div>
                <h4 class="font-black uppercase tracking-widest text-xs text-green-400 mb-6">Quick Actions</h4>
                <div class="flex space-x-4 mb-6">
                    <a href="{{ route('products') }}" class="w-10 h-10 rounded-full bg-green-800 flex items-center justify-center hover:bg-green-700 transition text-xs font-bold" aria-label="Browse products">P</a>
                    <a href="{{ route('services') }}" class="w-10 h-10 rounded-full bg-green-800 flex items-center justify-center hover:bg-green-700 transition text-xs font-bold" aria-label="Browse services">S</a>
                    <a href="{{ route('register') }}" class="w-10 h-10 rounded-full bg-green-800 flex items-center justify-center hover:bg-green-700 transition text-xs font-bold" aria-label="Become a vendor">V</a>
                </div>
                <p class="text-xs text-green-300 italic">Browse services, shop products, or join the marketplace as a vendor.</p>
            </div>
        </div>

        <div class="max-w-7xl mx-auto px-6 mt-16 pt-8 border-t border-green-800 text-center relative z-10">
            <p class="text-sm text-green-400">Copyright 2026 LandScapeHub. Built for Kenyan homeowners.</p>
        </div>
    </footer>
