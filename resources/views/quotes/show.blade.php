...
<div class="bg-gray-50 min-h-screen py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <a href="{{ route('quotes.index') }}" class="inline-flex items-center text-gray-500 hover:text-green-800 font-bold transition-all mb-4">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <span class="text-sm text-gray-500">Posted {{ $quote->created_at->diffForHumans() }}</span>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-white rounded-3xl shadow-xl overflow-hidden">
                    ...
                    <div class="h-80 w-full overflow-hidden">
                        <img src="{{ Storage::url($quote->image) }}" alt="Project image" class="w-full h-full object-cover">
                    </div>
                    @endif
                    <div class="p-8">
                        <div class="flex items-center text-gray-600 mb-6">
                            ...
                            <span class="text-lg font-bold">{{ $quote->location }}</span>
                        </div>
                ...
                <!-- Responses Section -->
                <div>
                    <h2 class="text-2xl font-black text-gray-900 mb-6">Vendor Proposals ({{ $quote->responses->count() }})</h2>

                    @if($quote->responses->isEmpty())
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white rounded-3xl shadow-md p-8 border hover:border-green-300 transition-all {{ $response->status === 'accepted' ? 'ring-2 ring-green-600 border-transparent bg-green-50' : '' }}">
                            <div class="flex flex-col md:flex-row justify-between gap-6">
                                <div class="flex-grow">
                                    <div class="flex items-center gap-3 mb-4">
                                        <div class="w-12 h-12 bg-green-100 rounded-2xl flex items-center justify-center text-green-800 font-black text-xl uppercase">
                                            {{ substr($response->vendor->business_name, 0, 1) }}
                                        </div>
                                        <div>
                                            <h4 class="text-xl font-black text-gray-900">{{ $response->vendor->business_name }}</h4>
                                            <span class="text-sm text-gray-500">Proposal sent {{ $response->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                    <p class="text-gray-700 leading-relaxed mb-4">{{ $response->description }}</p>
                                </div>
                                <div class="flex flex-col justify-between items-end gap-4 min-w-[200px]">
                                    <div class="text-right">
                                        ...
                                        <p class="text-3xl font-black text-green-800">KES {{ number_format($response->quoted_price) }}</p>
                                    </div>

                                    @if($quote->status === 'posted')
                                    <form action="{{ route('quotes.accept', $response) }}" method="POST">
                                        @csrf
